...
const postnews = async (newsData, id) => {
  ...
  try {
    ...
    if (id) {
      await axios.patch(
        `${process.env.VITE_BACKEND_URL}/news/${id}`,
        formData,
        config
      );
    } else {
      await axios.post(`${process.env.VITE_BACKEND_URL}/news`, formData, config);
    }
  ...
  return (
    <div className="p-4  ">
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="bg-[#EAF5FF]00 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
        >
          Submit
        </button>
      </form>
